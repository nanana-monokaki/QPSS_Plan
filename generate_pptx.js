...
async function createPresentation() {
    ...
    pres.defineSlideMaster({
        ...
        objects: [
            // Top curtain (緞帳)
            { image: { x: 0, y: 0, w: "100%", h: 0.5, path: "curtain_top.png" } },
            // Bottom curtain (緞帳)
            { image: { x: 0, y: 5.125, w: "100%", h: 0.5, path: "curtain_bot.png" } },
            // Slide Number
            { slideNumber: { x: 9.0, y: 5.35, w: 0.5, h: 0.2, fontSize: 10, color: "FFFFFF" } }
        ]
    });
    ...
    // Save
    await pres.writeFile({ fileName: "聞き耳アワーシリーズ企画書_改訂版.pptx" });
